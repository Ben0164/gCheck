...
public class MethodListActivity extends AppCompatActivity {
    ...
    private void loadData() {
        db.methodDao().getMethodsByActivity(activityId).observe(this, methods -> {
            adapter.setMethods(methods);
        });

        db.expenseDao().getTotalByActivity(1, activityId).observe(this, total -> {
            tvActivityTotal.setText(String.format(Locale.getDefault(), "₱%.2f", total != null ? total : 0.0));
        });
    }
    ...
    class MethodAdapter extends RecyclerView.Adapter<MethodAdapter.ViewHolder> {
        ...
        MethodAdapter(List<MethodEntity> methods) {
            this.methods = methods;
        }

        void setMethods(List<MethodEntity> methods) {
            this.methods = methods;
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            db.expenseDao().getTotalByMethod(1, method.getId()).observe(MethodListActivity.this, total -> {
