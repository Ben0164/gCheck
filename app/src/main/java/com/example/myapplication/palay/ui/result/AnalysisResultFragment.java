package com.example.myapplication.palay.ui.result;
...
public class AnalysisResultFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        btnTrackExpenses.setOnClickListener(v -> {
            ...
            // Using analysisId as batchId for this mock flow
            expenseArgs.putLong("BATCH_ID", analysisId);
            // Default yield for mock purposes if not provided
            expenseArgs.putDouble("YIELD_KG", 5000.0); 
            expenseFragment.setArguments(expenseArgs);

            if (getActivity() instanceof MainActivity) {
                MainActivity activity = (MainActivity) getActivity();
                activity.loadFragment(expenseFragment);
            }
        });
