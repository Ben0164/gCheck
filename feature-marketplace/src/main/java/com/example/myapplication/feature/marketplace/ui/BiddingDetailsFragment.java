...
public class BiddingDetailsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (toolbar != null) toolbar.setNavigationOnClickListener(v -> requireActivity().getOnBackPressedDispatcher().onBackPressed());
        if (batchId <= 0) {
    ...
    private void loadData() {
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            if (product == null) {
                if (isAdded()) requireActivity().runOnUiThread(() -> showError("Product not found"));
                return;
            }
            double yield = product.getQuantity();
            if (yield <= 0) {
                if (isAdded()) requireActivity().runOnUiThread(() -> showError("Invalid yield data"));
                return;
            }
            ...
            repository.getRankedBids(productId, product.getLatitude(), product.getLongitude(), rankedBids -> {
                if (isAdded()) {
                    requireActivity().runOnUiThread(() -> {
                        ...
                        if (rankedBids.isEmpty()) Toast.makeText(getContext(), "No buyer offers yet", Toast.LENGTH_SHORT).show();
                    });
                }
            });
        });
        ...
        if (millis > 0) {
            countDownTimer = new CountDownTimer(millis, 1000) {
                @Override public void onTick(long millisUntilFinished) {
                    ...
                    tvTimer.setText(String.format(Locale.getDefault(), "Time Left: %02d:%02d:%02d", secs/3600, (secs%3600)/60, secs%60));
                }
                @Override public void onFinish() { tvTimer.setText("Bidding Ended"); }
            }.start();
        } else {
    ...
    private void acceptBid(MarketplaceRepository.SmartBid smartBid) {
        repository.acceptBid(productId, smartBid.bid.getId(), message -> {
